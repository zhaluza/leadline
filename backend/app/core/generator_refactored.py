"""
AMT Backing Generator Core Module (Refactored)
=============================================

This module provides the core AMTBackingGenerator class for generating backing tracks
and lead melodies. This is a refactored version that removes unused methods and
...
class AMTBackingGenerator:
    ...
    def create_backing_track(self, num_bars=8, tempo=120):
        ...
        chord_progression = [
            [60, 64, 67],  # C major
            [69, 72, 76],  # A minor (transposed)
            [65, 69, 72],  # F major (transposed)
            [67, 71, 74],  # G major (transposed)
        ]
        ...
        for bar in range(num_bars):
            ...
            for note_pitch in chord:
                ...
                print(f"[DEBUG] Backing: Note pitch={note_pitch}, start={bar_start}, end={bar_start + seconds_per_bar}")
        ...
        print(f"[DEBUG] Backing track: actual MIDI tempos: {midi.estimate_tempos()}")
        return midi

    def create_backing_track_with_chords(self, chord_progression: list[str], key: str = "C", num_bars: int = 8, tempo: int = 120):
        """Create a backing track with custom chord progression."""
        ...
        midi.adjust_times(0, total_duration)
    ...
    def create_seed_melody(self, key=60, num_bars=8, tempo=120):
        """Create a seed melody for the first measure using diatonic scale notes."""
